                                ...
                                <ul className="projects-description--container">
                                    ...
                                    <li>
                                        ...
                                        <ul>
                                            ...
                                            <br/>
                                            <li><code className="token_reservada">Errors, Exceptions, e RuntimeExceptions:</code> 
                                                <ul className="-nestedInnerUl">
                                                    <li>Tratamento de erros;</li>
                                                    <li>Estrutura hierárquica (tudo no Java é Objeto, logo eles também possuem herança, todos extends de Throwable);</li>
                                            ...
                                            <li><code className="-main-moduleTitleLi token_reservada">Regex (Expressões Regulares) ou também Regexp (Regular Expression):</code> 
                                                <ul className="-nestedInnerUl">
                                                    <li>"Sublinguagem" utilizada para encontrar padrões em textos, os conceitos e representações dos caracteres especiais de configuração (meta caracteres) servem não apenas para o Java como é praticamente universal; Em particular no Java utilizamos duas (2) classes especializadas para este contexto sendo elas o Pattern que representa o padrão/máscara em search utilizando esses caracteres especiais de configuração e o Matcher para realizar a busca e comparação match em sí da expressão no texto; Um exemplo de utilidade para elas é identificar URL's "http://www.wubalubadubdub.rick.morty.com" aonde toda URL tem essa máscara "(www|http:|https:)+[^\s]+[\w]", essa expressão regex é a mais simples para URL's não sendo ideal para ambiente de produção;</li>
                                                    <li>Meta caracteres (Caracteres de configuração sobre o padrão em busca);</li>
                                                    <li>Range (Intervalo) de valores com `[]`;</li>
                                                    <li>Quantificadores quando desejamos retornar vários valores do range `[]`;</li>
                                            ...
                                            <li><code className="-main-moduleTitleLi token_reservada">Introdução a Programação Funcional - Parametrizando Comportamentos:</code> 
                                                <ul className="-nestedInnerUl">
                                                    <li>Delegamos as responsabilidades sobre as regras de comparações ou outros comportamentos em alguns métodos para o chamador realizando assim uma inversão de controle, assim tornamos esse método mais genérico (Por exemplo `findByName()` que pode facilmente ser copiado e colado como `findByOutroAtributo()` percebemos que a única mudança nesta implementação será as comparações do `if` logo podemos extrair em um método mais genérico `find(Predicate predicate)` aonde Predicate É uma interface que representa um predicado sendo ele uma função que recebe um argumento e retorna um valor booleano);</li>
                                                    <li>Além desta Interface Funcional também possuem outras com diferentes Target Types e Functional Descriptor (Mais explicações abaixo ou no módulo sobre Funções Lambdas, basicamente é a assinatura do método ÚNICO desta Interface Funcional, ou seja, isto se refere aos tipos de entradas e retornos deste método em questão, isso é importante pois devido a sintaxe dos corpos das Lambdas não precisar explicitar os tipos de dados que estamos trabalhando, então essa sintaxe deve respeitar a assinatura do método em questão para que a JVM possa executar sem a necessidade de especificar manualmente esses tipos de entradas e retornos neste corpo da Lambda).</li>
                                                    <li>E é graças a esses Target Types e Functional Descriptors ditos anteriormente, que podemos aplicar a sintaxe das Funções Lambdas em praticamente qualquer classe ou interface (Mesmo não sendo anotadas com @FunctionalInterface) desde que elas possuam métodos ÚNICOS definidos, e desde que respeitemos a assinatura deste método em questão sobre os tipos de entrada e retorno (Isso é o Target Type (Tipo alvo é o tipo de dado esperado em um contexto onde uma expressão lambda, uma referência a método ou uma classe anônima são usados.) e o Functional Descriptor (Que é um termo usado para descrever a assinatura de um método funcional, ou seja, a lista de argumentos e o tipo de retorno do método.));</li>
                                            ...
                                            <li><code className="-main-moduleTitleLi token_reservada">Programação Funcional - Method Reference:</code> 
                                                <ul className="-nestedInnerUl">
                                                    ...
                                                    <li>Referência a método de instância de um tipo de objeto arbitrário: `String::length;` Obs: Esse parece meio confuso, mas é simples, ele é utilizado quando vamos executar esse referência em objetos que ainda não existem porisso "arbitrário", mas iram ser criados em algum momento futuro;</li>
                                            ...
                                            <li><code className="-main-moduleTitleLi token_reservada">Programação Funcional - Streams (Fluxo de dados):</code> 
                                                <ul className="-nestedInnerUl">
                                                    ...
                                                    <li>Também abordamos o problema dos Wrappers dos Tipos primitivos sobre encapsular e desencapsular a cada iteração, alocando e re-alocando memória quando utilizado o Stream genérico (Stream&#60;Double&#62;), tornando esse processo computacionalmente custoso, então é utilizado as classes especializadas para trabalhar com fluxos de dados relacionados aos Tipos primitivos e Wrappers deles (Exemplo: DoubleStream) que evitam esse problema;</li>
                                                    <li>Também explicamos as diferenças entre os métodos de transformação `.map()` e `.flatMap()` que realizam a mesma função, porém o `.flatMap()` é utilizado quando temos coleções multidimensionais aninhas umas as outras, esse tipo de coleção gera Streams de Streams (Stream&#60;Stream&#60;Stream&#60;Anime&#62;&#62;&#62;) deixando a operação mais complexa então devemos "desachatar" manualmente se utilizado o `.map()` comum aumentando o número de operações neste fluxo, diferente do `.flatMap()` que já o faz para nós, a implementação em baixo nível dele basicamente une os Streams em apenas um, já retornando apenas um fluxo contendo todos os elementos que eram aninhados neste mesmo fluxo de saída (Stream&#60;Anime&#62;);</li>
